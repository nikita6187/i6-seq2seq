...
# TODO: manage these variables
last_encoder_state = None
last_transducer_state = None
...
def run_new_block(session, block_inputs, previous_alignments, block_index, transducer_width, targets, total_blocks,
    ...
    # TODO: Note that the new alignments need to be maximum length of target                                [p, recalc]
    # TODO: Manage new alignements
    ...
    def run_transducer(session, transducer_state, encoder_outputs, transducer_width):
        ...
        :return: Transducer logits [transducer_amount_outputs, batch_size(1), vocab_size],
    ...
    block_encoder_outputs, last_encoder_state = run_encoder(session, last_encoder_state, block_inputs)
    ...
    for i in range(len(previous_alignments)):
        ...
        min_index = alignment.alignment_position[0] + \
                    max(0, targets_length - (
                            (total_blocks - block_index) * transducer_width) + transducer_width)  # TODO: calculate this
        max_index = alignment.alignment_position[0] + transducer_width + min(0, targets_length - (
                alignment.alignment_position[0] + transducer_width))
